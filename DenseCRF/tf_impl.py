...
class HashTable:
    def __init__(self, key_size, n_elements):
        self.key_size_ = key_size # size_t
        self.filled_ = 0 # size_t
        self.capacity_ = 2 * n_elements # size_t
        ...
        for k in range(self.N_):
            ...
            for i in range(self.d_ + 1):
                ...
                if rank[i] < 0:
                    ...
                    rank[i] -= self.d_ + 1
                    rem0[i] -= self.d_ + 1
            rank_ori, rem0_ori = rank.copy(), rem0.copy()
            # # ->> Numpify
            # rank[:] += _sum
            # cond_mask1 = rank < 0
            # rank[cond_mask1] += self.d_ + 1
            # rem0[cond_mask1] += self.d_ + 1
            # cond_mask2 = rank > self.d_
            # rank[cond_mask2] -= self.d_ + 1
            # rem0[cond_mask2] -= self.d_ + 1
            # print(np.allclose(rank_ori, rank))
            # print(np.allclose(rem0_ori, rem0))
