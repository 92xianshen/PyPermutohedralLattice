...
class HashTablePermutohedral(object):
    ...
    def lookup(self, k, create=True):
        ...
        create : boolean, default True
            whether a new entry should be created if nothing is matched
...
class PermutohedralLattice(object):
    ...
    def __init__(self, d, vd, inp_len):
        """
        Initialize a new lattice object
        ...
        # ->> 2021.08.26 Numpify
        self.canonical = self.canonical.reshape((self.d1, self.d1))
        for i in range(self.d1):
            self.canonical[i, :self.d1 - i] = i
            self.canonical[i, self.d1 - i:] = i - self.d1
        self.canonical = self.canonical.flatten()
        ...
        # ->> 2021.08.27 Numpify
        self.scale_factor[:] = expected_std / np.sqrt((np.arange(self.d) + 1) * (np.arange(self.d) + 2))
    ...
    def splat(self, position, pos_idx, value):
        ...
        # ->> 2021.08.27 Numpify
        # E = np.vstack([
        #     np.ones((self.d, ), dtype='float32'), 
        #     np.diag(-np.arange(self.d, dtype='float32') - 2) + np.triu(np.ones((self.d, self.d), dtype='float32'))])
        # curr_scale_factor = np.diag(self.scale_factor)
        # curr_position = position[pos_idx:pos_idx + self.d][:, np.newaxis]
        # self.elevated[:] = np.dot(np.dot(E, curr_scale_factor), curr_position).flatten()
